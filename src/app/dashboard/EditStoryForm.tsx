...
export function EditStoryForm({ story, worldId }: EditStoryFormProps) {
  ...
  return (
    <form action={formAction} className="space-y-4">
      ...
      <div>
        <label htmlFor="edit-story-title" className="mb-1.5 block text-xs font-medium text-zinc-400">
          Title
        </label>
        <input
          id="edit-story-title"
      ...
      <div>
        <label htmlFor="edit-story-summary" className="mb-1.5 block text-xs font-medium text-zinc-400">
          Summary
        </label>
        <textarea
          ...
          className="w-full resize-y rounded-lg border border-white/10 bg-[#141416] px-3 py-2 text-sm text-zinc-100 outline-none focus:border-violet-500/50"
        />
      </div>

      <div>
        <label htmlFor="edit-story-status" className="mb-1.5 block text-xs font-medium text-zinc-400">
          Status
        </label>
        <select
      ...
      <div>
        <label htmlFor="edit-story-project-type" className="mb-1.5 block text-xs font-medium text-zinc-400">
          Project type
        </label>
        <select
      ...
      <button
        type="submit"
        disabled={pending}
        className="rounded-lg bg-white/[0.06] px-4 py-2 text-sm font-medium text-zinc-200 transition hover:bg-white/10 disabled:opacity-50"
      >

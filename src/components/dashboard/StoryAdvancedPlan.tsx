...
type StoryAdvancedPlanProps = {
  children: ReactNode;
};

export function StoryAdvancedPlan({ children }: StoryAdvancedPlanProps) {
  return (
    <details
      id="story-advanced-plan"
        ...
        <div className="flex items-center justify-between gap-3">
          <div>
            <h2 className="text-sm font-semibold uppercase tracking-wider text-[var(--brand-text-secondary)]">
              {CREATOR_STORY.advancedPlanLabel}
            </h2>
            <p className="mt-1 text-xs text-[var(--brand-text-secondary)]">
              {CREATOR_STORY.advancedPlanHint}
            </p>
          </div>
          <span
